...
import { HeadersLoai } from "../componentLoai/headerLoai";

const fetchData = async (token, rank) => {
  try {
...
function EditSpecies() {
  ...
  const fetchAllData = async () => {
    try {
      if (renderData) {
        const [kingdom, phylum, classData, order, family, genus] =
          await Promise.all([
            fetchData(renderData, "Kingdom"),
            fetchData(renderData, "Phylum"),
            fetchData(renderData, "Class"),
            fetchData(renderData, "Order"),
            fetchData(renderData, "Family"),
            fetchData(renderData, "Genus"),
          ]);
        setListKingdom(kingdom);
        setListPhylum(phylum);
        setListClass(classData);
        setListOrder(order);
        setListFamily(family);
        setListGenus(genus);
      }
    } catch (error) {
      console.error(error);
  ...
  const fetchSpeciesData = async () => {
    try {
      if (renderData && id) {
        ...
        setTen(species.ten || "");
        setTenKhoaHoc(species.ten_khoa_hoc || "");
        setTenTacGia(species.ten_tac_gia || "");
        setTenDiaPhuong(species.ten_dia_phuong || "");
        setNguon(species.nguon_du_lieu || "");
        setKingdom(species.kingdom_id || "");
        setPhylum(species.phylum_id || "");
        setClass(species.class_id || "");
        setOrder(species.order_id || "");
        setFamily(species.family_id || "");
        setGenus(species.genus_id || "");
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleAddUser = (e) => {
    ...
    const updatedSpecies = {
      ...
      toa_dos: speciesData.toa_dos || [],
    };
    axios
      .post(`${url}/${id}`, updatedSpecies, {
        headers: {
          Authorization: `Bearer ${renderData}`,
        },
      })
